...
\\ \hline
\textrm{Values use offset to index}&0&0&1&0&Bits[26,24]&&&Bits[23,0]
\\ &&&&& \quad\textrm{0b000 f64 literal}&&&\textrm{offset to base address}
\\ &&&&& \quad\textrm{0b001 f64 allocated with dynamic array}&&&\textrm{offset in array}
\\ &&&&& \quad\textrm{0b010 symbol literal}&&&\textrm{offset to base address}
\\ &&&&& \quad\textrm{0b011 symbol allocated with dynamic array}&&&\textrm{offset in array}
\\ \hline
\textrm{Direct enum value}&0&0&1&1&0&0&0&Bits[23,16]&Bits[15,0]
...
\\ &&&&&&&&\quad\textrm{0x010000-0x5FFFFF}
\\ &&&&&&&&\quad\quad\textrm{short narrow string literal}
\\ &&&&&&&&\quad\textrm{0x600000-0xAFFFFF}
\\ &&&&&&&&\quad\quad\textrm{short wide string literal}
\\ &&&&&&&&\quad\textrm{0xB00000-0xFEFFFF}
\\ &&&&&&&&\quad\quad\textrm{other string literal}
\\ \hline
...
Primitive value can not reference to other value
...
String primitive value composite of the following layout, this design copied idea from `quickjs`,
...
struct berry_string {
    ...
    uint64_t len: 53;
#else
#error "BERRY_MEM_PROFILE should be defined" 
#endif
    ...
    union {
        ...
        uint16_t str16[0]; /* 16 bit strings will also get an extra null terminator */
    } u;
};
...
* Reference counter
* String length
* String type(wide or narrow)
* String content

String literal store  

* 0x010000-0x5FFFFF short narrow string literal
...
* 0x600000-0xAFFFFF short wide string literal
  It's record the offset of the wide string literal, and
  it's composite the 2 byte length header and the literal body.
  So the length of this string literal are always the multiple of 2, so the
  `offset=(berry_value&mask - 0x600000)*2`. Every str
* 0xB00000-0xFEFFFF other string literal
  All string literal can not represent with `short narrow string literal`
  or `short wide string literal` because of length is too big or offset index
  are too big, then using `other string literal` to storage it.
  It's an array of the following struct. The maximal length of string literal
  are `2^31-1`

```c
  struct jerry_string_literal {
      uint32_t len : 31;
      uint8_t is_wide_char : 1; /* 0 = 8 bits, 1 = 16 bits characters */
      union {
          uint8_t str8[];
