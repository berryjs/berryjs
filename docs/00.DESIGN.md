...
Bit[23,16]&Bits[15,0]
\\ \textrm{This bit are used to distinguish}
\\ \textrm{normal value and error value:}
\\ \quad\textrm{0 stands for normal value}
\\ \quad\textrm{1 stands for error value}
\\ \hline
\textrm{Direct integer number value}&0&0&0&Bits[27,0]
\\ \textrm{the length is 28 bits}&&&& \textrm{number body}
\\ \hline
\textrm{Direct f64 literal}&0&0&1&0&0&0&0&Bits[23,0]
\\ &&&&&&&&\quad\textrm{index in f64 literal list}
\\ \hline
\textrm{Allocated f64 value}&0&0&1&0&0&0&1&Bits[23,0]
\\ &&&&&&&&\quad\textrm{index in f64 dynamic list}
\\ \hline
\textrm{Direct enum value}&0&0&1&0&0&1&0&\textrm{0x00}&Bits[15,0]
\\ &&&&&&&&&\quad\textrm{0 uninitialized}
\\ &&&&&&&&&\quad\textrm{1 undefined}
\\ &&&&&&&&&\quad\textrm{2 false}
\\ &&&&&&&&&\quad\textrm{3 true}
\\ &&&&&&&&&\quad\textrm{4 null}
\\ &&&&&&&&&\quad\textrm{may record error code here}
\\ &&&&&&&&&\quad\textrm{max enum is 0xFFFF }
\\ \hline
\textrm{Reserved}&0&0&1&0&0&1&0&\textrm{0x01-0xFF}
\\ &0&0&1&0&0&1&1&Bits[23,0]
\\ \hline
\textrm{Primitive symbol allocated}&0&0&1&0&1&Bits[25,0]
\\ &&&&&&\quad\textrm{index in dynamic symbol list}
\\ \hline
\textrm{String literal store}&0&0&1&1&0&0&0&Bits[23,0]
\\ &&&&&&&&\quad\textrm{0x000000-0x3FFFFF}
\\ &&&&&&&&\quad\quad\textrm{short narrow string literal}
\\ &&&&&&&&\quad\textrm{0x400000-0x7FFFFF}
\\ &&&&&&&&\quad\quad\textrm{short wide string literal}
\\ &&&&&&&&\quad\textrm{0x800000-0xBFFFFF}
\\ &&&&&&&&\quad\quad\textrm{general string literal}
\\ \hline
\textrm{Sliced string store}&0&0&1&1&0&0&0&Bits[23,0]
\\ &&&&&&&&\quad\textrm{0xC00000-0xFEFFFE}
\\ &&&&&&&&\quad\quad\textrm{allocated sliced string}
\\ \hline
\textrm{Empty string literal}&0&0&1&1&0&0&0&Bits[23,0]
\\ &&&&&&&&\quad\textrm{0xFEFFFF}
\\ \hline
\textrm{Direct wide string value}&0&0&1&1&0&0&0&Bits[23,16]&Bits[15,0]
\\ \textrm{the length is constant 1}&&&&&&&&\textrm{constant 0xFF}&\textrm{wide char}
\\ \hline
\textrm{Direct narrow string value}&0&0&1&1&Bit[26,24]&&&Bits[23,0]
\\ \textrm{the length is range [1-3] on 32bit arch}&&&&&\textrm{0b001-0b011}&&&\textrm{char array}
\\ \textrm{the length is range [1-7] on 64bit arch}&&&&&\textrm{0b001-0b111}
\\ \hline
\textrm{Primitive value: string}&0&1&Bits[28,0]
\\ \hline
\textrm{Composite value, possible values:}&1&0&Bits[28,0]
\\ \quad\textrm{object value}
\\ \quad\textrm{bigint value}
\\ \quad\textrm{records\&tuples value}
\\ \hline
\textrm{Primitive value: number}&1&1&Bits[28,0]
\\ \hline
\end{array}
$$
...
* short wide string literal
  It's record the offset of the wide string literal, and it's composite the 2 byte length header and the literal body. So the length of this string literal are always the multiple of 2. This is the structure of `short wide string literal`:
...
* general string literal
  All string literal can not represent with `short narrow string literal` or `short wide string literal` because of length is too big or offset index are too big, then using `general string literal` to storage it. The maximal length of general string literal are `2^32-1`. It's an concat list of the following struct:

  ```c
  struct berry_string_literal_general {
    uint32_t len;
    /* 0 = 8 bits, 1 = 16 bits characters */
    uint8_t is_wide_char :1; /* Using offset_30 */

    /*
     * Using offset_64 when is_wide_offset=1, otherwise
     * using offset_30, this happens only when
     * total literal store bigger than 2^30 bytes
     * this would rarely happened, and only usable 
     * in BERRY_MEM_PROFILE==64. And for other code
     * path it's disabled.
     */
    uint8_t is_wide_offset :1;
    union {
      uint32_t offset_30: 30;
  #if BERRY_MEM_PROFILE == 64
      uint64_t offset_64: 62;
  #endif
    } u;
  };
  ```
