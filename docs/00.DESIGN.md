...
\\ &&&&&&&&\quad\textrm{0x010000-0xFEFFFF}
\\ &&&&&&&&\quad\textrm{offset in the literal store}
\\ \hline
...
\\ \hline
\textrm{Composite value:}&1&0&Bits[28,0]
...
#### Primitive value `string`
  ...
  String primitive value composite of
...
Even though the encoding of `number` is not consecutive, but `Primitive value: number`
are start with `11`, and `Direct integer number value` starts with `00`, so in fact
they are consecutive in other manner(circle), we only need one branch to judge if
a `berry_value` is a number.
